...
local function kq_42fn(inst, reader)
    ...
    local reveal_center = Vector3(x, 0, z) + Vector3(150 * math.cos(rotation*DEGREES), 0, -150 * math.sin(rotation * DEGREES) )
...
local function SpawnNewLeifs(x, y, z, doer, num_spawns)
    local ents = TheSim:FindEntities(x, y, z, TUNING.LEIF_IDOL_SPAWN_RADIUS, { "tree" }, LEIFTARGET_CANT_TAGS, LEIFTARGET_ONEOF_TAGS)
    for i, ent in ipairs(ents) do
