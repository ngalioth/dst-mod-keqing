...
local function DoAttack(inst, tx, ty, tz, owner, weapon, specialmult, overridemult, overrideradius)
    ...
    -- 出伤
    if inst ~= nil and inst:IsValid() and owner ~= nil and owner:IsValid() and owner.components.health ~= nil and
        not owner.components.health:IsDead() and owner.components.combat ~= nil then
        local exclude_tags = {"INLIMBO", "companion", "wall", "abigail", "shadowminion", "player", "structure"}
        local ents = TheSim:FindEntities(tx, ty, tz, overrideradius or TUNING.ACHERON_SKILL_RANGE, {"_combat"},
            exclude_tags)
