local assets = {Asset("ANIM", "anim/kq_hairpins.zip"), Asset("IMAGE", "images/inventoryimages/kq_hairpins.tex"),
...
local function DoPercentDmg(inst, attacker)
    if attacker.target ~= nil then
        ...
        if inst ~= nil and inst.components ~= nil and inst.components.inventory ~= nil then
            if target.components.health ~= nil then
                target.components.health:DoDelta(-target.components.health.maxhealth * 0.002, nil, inst.prefab, nil,
                    inst, true)
            end
        end
    end
...
local function onequip(inst, owner) -- 装备的函数
    ...
    if inst["nohit"] == 1 then -- 防击飞和地皮减速（？）
        ...
        inst.components.armor:InitIndestructible(.90)
...
local function fn()
    ...
    inst.components.armor:InitIndestructible(.50)
